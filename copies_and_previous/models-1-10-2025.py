...
class MSH_Model(db.Model):
    ...
    field_separator = Column(String(1), nullable=False)  # MSH-1
    encoding_characters = Column(String(5), nullable=False)  # MSH-2
    ...
    date_time_of_message = Column(TIMESTAMP(timezone=True), nullable=False)  # MSH-7 
    security = Column(String, nullable=True)  # MSH-8
    message_type = Column(String, nullable=False)  # MSH-9
    message_control_id = Column(String(199), nullable=False)  # MSH-10
    processing_id = Column(String, nullable=False)  # MSH-11
    version_id = Column(String, nullable=False)  # MSH-12
...
class PID_Model(db.Model):
    ...
    patient_identifier_list = Column(ARRAY(String), nullable=False)  # PID-3
    alternate_patient_id = Column(String, nullable=True)  # PID-4
    patient_name = Column(ARRAY(String), nullable=False)  # PID-5
    ...
    multiple_birth_indicator = Column(Boolean, nullable=True)  # PID-24
    ...
    patient_death_indicator = Column(Boolean, nullable=True)  # PID-30
    identity_unknown_indicator = Column(Boolean, nullable=True)  # PID-31
    identity_reliability_code = Column(ARRAY(String), nullable=True)  # PID-32
...
class PV1_Model(db.Model):
    ...
    patient_class = Column(String, nullable=False)  # PV1-2
...
class ORC_Model(db.Model):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    unparsed_msg_id = Column(Integer, ForeignKey('unparsed_hl7.id'))
    order_control = Column(String, nullable=False)  # ORC-1
...
class OBR_Model(db.Model):
    ...
    universal_service_identifier = Column(Text, nullable=False)  # OBR-4
...
class OBX_Model(db.Model):
    ...
    observation_identifier = Column(Text, nullable=False)  # OBX-3: Observation Identifier
    ...
    observation_result_status = Column(String, nullable=False)  # OBX-11: Observation Result Status
