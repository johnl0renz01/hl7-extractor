...
def populateSegment(obj, segment, segment_str):
    ...
    def extractSegment(idx = 0):
            segment_value = None
            try:
                curr_segment = f"{segment_str}_{index + 1}" # get the current segment to parse
                val = getattr(segment[idx], curr_segment).value # get the value
                if '^' in val and val != '^~\&': # if current component has caret
                    ...
                    for i in range (total_subcomponents):
                        ...
                        temp_val.append(getattr(getattr(segment[idx], curr_segment),temp_curr_segment).value) #store in array 

                    segment_value = temp_val #assign array to current segment key
                else:
                    try:
                        if len(getattr(segment[idx], curr_segment)) > 1: # if the segment is greater than 1
                            if type(getattr(segment[idx], curr_segment)) != str: # if the segment is list
                                val = [segment_field.value for segment_field in getattr(segment[idx], curr_segment)] # split all elements and put in array
                                val = list(filter(lambda txt: txt != "", val)) # remove empty items
                    except Exception:
                        pass
    ...
    for index, key in enumerate(obj.keys()): # iterate on all keys 
        ...
        if len(segment) > 1:
            if type(segment) != str:
                ...
                segment_items = len(segment)

        if multiple_segment:
            for _ in range(segment_items):
                segment_values.append(extractSegment(_))

            obj[key] = segment_values
        else:
            obj[key] = extractSegment()
